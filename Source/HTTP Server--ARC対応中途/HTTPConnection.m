...
- (NSString *)generateNonce
{
	...
	//ARC//NSString *newNonce = [NSMakeCollectable(CFUUIDCreateString(NULL, theUUID)) autorelease];
...
- (BOOL)isAuthenticated
{
	...
	if([self useDigestAccessAuthentication])
	{
		...
		//ARC//NSString *method = [NSMakeCollectable(CFHTTPMessageCopyRequestMethod(request)) autorelease];
		NSString *method = (__bridge_transfer NSString*)CFHTTPMessageCopyRequestMethod(request);
		
		//ARC//NSURL *absoluteUrl = [NSMakeCollectable(CFHTTPMessageCopyRequestURL(request)) autorelease];
		NSURL *absoluteUrl = (__bridge_transfer NSURL*)CFHTTPMessageCopyRequestURL(request);
...
- (void)replyToHTTPRequest
{
	...
	//ARC//NSString *version = [NSMakeCollectable(CFHTTPMessageCopyVersion(request)) autorelease];
	...
	//ARC//NSURL *uri = [NSMakeCollectable(CFHTTPMessageCopyRequestURL(request)) autorelease];
	...
	//ARC//NSString *rangeHeader = [NSMakeCollectable(CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Range"))) autorelease];
	NSString *rangeHeader = (__bridge_transfer NSString*)CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Range"));
...
- (CFHTTPMessageRef)prepareMultiRangeResponse:(UInt64)contentLength
{
	...
	CFUUIDRef theUUID = CFUUIDCreate(NULL);
	//ARC//ranges_boundry = NSMakeCollectable(CFUUIDCreateString(NULL, theUUID));
...
- (NSData *)preprocessResponse:(CFHTTPMessageRef)response
{
	...
	//ARC//NSData *result = NSMakeCollectable(CFHTTPMessageCopySerializedMessage(response));
	NSData *result = (__bridge_transfer NSData*)CFHTTPMessageCopySerializedMessage(response);
...
- (BOOL)onSocketWillConnect:(AsyncSocket *)sock
{
	if([self isSecureServer])
	{
		...
		if([certificates count] > 0)
		{
			...
			CFDictionaryAddValue((__bridge CFMutableDictionaryRef)settings,
								 kCFStreamSSLIsServer, kCFBooleanTrue);
			
			CFDictionaryAddValue((__bridge CFMutableDictionaryRef)settings,
								 kCFStreamSSLCertificates, (__bridge CFArrayRef)certificates);
			...
			CFReadStreamSetProperty([asyncSocket getCFReadStream],
									kCFStreamPropertySSLSettings, (__bridge CFDictionaryRef)settings);
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	if(tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
			...
			//ARC//NSString *contentLength = [NSMakeCollectable(
			//										CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Content-Length"))) autorelease];
			NSString *contentLength = (__bridge_transfer NSString*)CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Content-Length"));
