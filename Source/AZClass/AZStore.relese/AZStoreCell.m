...
{	// ここは初期化時にしか通らない。　更新処理は、refresh にて
	[ibBuRestore setTitle:AZLocalizedString(@"AZStore Restore", nil) forState:UIControlStateNormal];
	[ibBuBuy setTitle:AZLocalizedString(@"AZStore Buy", nil) forState:UIControlStateNormal];
}
...
- (void)refresh
{
	...
	if ([kvs boolForKey: self.ppProduct.productIdentifier]) {
		...
		ibLbDetail.text = AZLocalizedString(@"AZStore Purchased", nil);
		ibBuRestore.hidden = YES;
		ibBuBuy.hidden = YES;
	} else {
		// Price 金額単位表示する
