...
// NSPropertyListSerialization ---> NSDictionary
					...
					target:(id)target
{
	...
	NSString *base64receipt = [self encodeBase64:(uint8_t *)receiptData.bytes length:receiptData.length];
    
    
	
	if (sharedSecret==nil) {
		GA_TRACK_ERROR(@"sharedSecret==nil")
		return NO;
	}
    // Create the POST request payload.
    NSString *payload = [NSString stringWithFormat:
						 @"{\"receipt-data\" : \"%@\", \"password\" : \"%@\"}",  
						 base64receipt, sharedSecret];
    
    NSData *payloadData = [payload dataUsingEncoding:NSUTF8StringEncoding];
    
#ifdef DEBUG
    NSString *serverURL = ITMS_SANDBOX_VERIFY_RECEIPT_URL;	//testing against the sandbox.
#else
...
- (BOOL)isTransactionAndItsReceiptValid:(SKPaymentTransaction *)transaction
{
    ...
    NSDictionary *receiptDict       = [self dictionaryFromPlistData:receiptData];
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    long verifyReceiptStatus = [status integerValue];
    ...
    if ([[UIDevice currentDevice] respondsToSelector:NSSelectorFromString(@"identifierForVendor")]) // iOS 6?
    {
        ...
        // Pre iOS 6 
        //1.0.7//NSString *localIdentifier           = [UIDevice currentDevice].uniqueIdentifier;
...
{	// レシート要求の結果が戻ってくる
	...
	if ([mDelegate respondsToSelector:@selector(verificationResult:)]) {
		[mDelegate verificationResult: isOk];
	}
}
...
- (NSString *)encodeBase64:(const uint8_t *)input length:(NSInteger)length
{
    ...
    //return nil;
	NSData *dt = [NSData dataWithBytes:input length:length];
	return [dt stringEncodedWithBase64];
}
...
- (NSString *)decodeBase64:(NSString *)input length:(NSInteger *)length
{
	//#warning Replace this method.
    //return nil;
	NSData *dt = [NSData dataWithBase64String:input];
...
{ 
	//return NULL; 
	NSData *dt = [NSData dataWithBytes:buf	length:size];
	return (char *)[[dt stringEncodedWithBase64] cStringUsingEncoding:NSASCIIStringEncoding];
}*/
...
{
	//return NULL; 
	NSString *zz = [NSString stringWithCString:s encoding:NSASCIIStringEncoding];
	NSData *dt = [NSData dataWithBase64String:zz];
	*data_len = [dt length];
	return (void *)[dt bytes];
}
